...
def pretrade_gate(payload: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    native_score = _num(native.get("score"),0.0) or 0.0 if isinstance(native, dict) else 0.0
    native_state = str((native.get("state") if isinstance(native, dict) else "") or "UNKNOWN")
    ...
    if entry_invalidated and tactical_horizon:
        ...
        if native_available and native_sign == sign and native_score >= 0.58:
            timing_multiplier = 0.65 if horizon == "1d" else 0.85
            soft.extend(["lower_timeframe_entry_invalidation","native_horizon_confirms_thesis"])
            entry_scope = "STRATEGIC_NATIVE_CONFIRMED"
        elif native_available and native_sign == -sign and native_score >= 0.58:
            timing_multiplier = 0.40 if horizon == "1d" else 0.60
            soft.extend(["lower_timeframe_entry_invalidation","horizon_structure_conflict"])
            entry_scope = "STRATEGIC_NATIVE_CONFLICT"
        else:
    ...
    decision_trend_sign = (native_sign if strategic_horizon and native_available and native_score>=0.45 else trend_sign)
    if decision_trend_sign and decision_trend_sign != sign: soft.append("horizon_structure_conflict" if strategic_horizon and native_available else "trend_direction_conflict")
    ...
    falsification = 0.0
    if strategic_horizon and native_available and native_sign and native_sign != sign:
        falsification += 0.24 * _clip(native_score/0.75)
    elif tactical_horizon and trend_sign and trend_sign != sign:
        falsification += 0.24
    ...
    if data_hard:
        ...
        gate_class="PASS"
    independent_soft=[x for x in soft if x not in ("lower_timeframe_entry_invalidation","entry_resolution_not_explicitly_lower_timeframe","native_horizon_confirms_thesis")]
    ...
    elif timing_multiplier < 1.0 and native_available and native_sign == sign and native_score>=0.58: entry_status="LOWER_TF_CAUTION_NATIVE_CONFIRMED"
    elif timing_multiplier < 1.0 and native_available and native_sign == -sign and native_score>=0.58: entry_status="HORIZON_CONFLICT"
    elif timing_multiplier < 1.0: entry_status="LOWER_TF_INVALIDATED"
    elif entry_q in ("LATE_EXTENDED","EXTENDED_WAIT_PULLBACK") or uncertainty>=0.70: entry_status="LATE_OR_WAIT"
    ...
    return {
        ...
        "native_horizon_available":native_available,"native_horizon_direction":native.get("direction") if isinstance(native,dict) else None,
        "native_horizon_score":round(native_score,4),"native_horizon_state":native_state,
...
def parameter_audit(signals: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    if {'trend_phase','trend_direction','intraday_structure','horizon_structure','horizon_structure_direction','trade_plan'} & keys: families.add('структура рынка')
...
def investor_asset_view(signals: Sequence[Mapping[str, Any]], model_agents: Optional[int]=None) -> Dict[str, Any]:
    ...
    for asset,rows in sorted(by_asset.items()):
        ...
        native_map={}
        for rr in rows:
            hh=str(rr.get('horizon') or ''); hs=rr.get('horizon_structure') or {}
            nd=_direction_sign(hs.get('direction') if isinstance(hs,dict) else rr.get('horizon_structure_direction'))
            native_map[hh]='↑' if nd>0 else '↓' if nd<0 else '→'
        fresh_strong=next((r for r in rows if r.get('signal_tier') in ('SUPER_LONG','SUPER_SHORT') and (r.get('trade_plan') or {}).get('fresh_breakout')),None)
        institutional=max(rows,key=lambda r:float(((r.get('institutional_signal') or {}).get('breakout_quality') or {}).get('quality_score') or 0.0),default={})
        inst_label=(institutional.get('institutional_signal') or {}).get('investor_signal')
        investor_signal=(inst_label if inst_label and inst_label!='WAIT' else ('STRONG BUY' if fresh_strong and fresh_strong.get('signal_tier')=='SUPER_LONG' else
                         'STRONG SELL' if fresh_strong and fresh_strong.get('signal_tier')=='SUPER_SHORT' else
                         'BUY' if sign>0 else 'SELL' if sign<0 else 'WAIT'))
        items.append({'asset':asset,'arrow':arrow,'trend':trend,'investor_signal':investor_signal,'weighted_direction_score':round(score,4),
                      'horizons':{h:hmap.get(h,'→') for h in ('1h','4h','1d','3d','7d')},
                      'native_horizons':{h:native_map.get(h,'→') for h in ('1h','4h','1d','3d','7d')},
                      ...
                      'institutional_signal':(institutional.get('institutional_signal') or {}),
