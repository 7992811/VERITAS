...
def _repair_bootstrap_backtest_source(src):
    start=src.find("def run_bootstrap_backtest(")
    if start<0:
        return src
    end=src.find("\ndef backtest_status():",start)
    if end<0:
        return src
    clean=CLEAN_BOOTSTRAP_BACKTEST_SOURCE.rstrip()+"\n\n"
    return src[:start]+clean+src[end+1:]
...
'''
    ...
    # Compile-safe repair: v80's historical text patches can occasionally leave
    # run_bootstrap_backtest malformed on a clean Render build. Restore the verified
    # source copy before compile/verification; live web decisions do not depend on it.
    dst=_repair_bootstrap_backtest_source(dst)
    if dst!=src:
        _write(TARGET,dst)
    return applied
...
# --- VERITAS v9.0 emergency PostgreSQL cleanup 2026-09-26 ---
# Preserve authoritative trades, positions, outcomes, decisions and learning.
# Remove only rebuildable high-churn telemetry/snapshots and repeated meta signals.
def _v90_emergency_pg_cleanup():
    import os as _os, json as _json
    _marker = "maintenance.emergency_cleanup_2026_09_26_v1"
    _url = _os.getenv("DATABASE_URL","").strip()
    if not _url:
        return
    try:
    ...
    def _elog(event, **kw):
        try:
            print(_json.dumps({"event":event, **kw}, ensure_ascii=False), flush=True)
        except Exception:
            pass
    try:
        with _psycopg.connect(_url, autocommit=True, row_factory=_dict_row) as _c:
            try:
                _r=_c.execute("SELECT value FROM system_settings WHERE key=%s LIMIT 1",(_marker,)).fetchone()
                if _r:
                    _elog("db_cleanup_skip",reason="already_completed",marker=_marker)
                    return
            except Exception:
                pass
            ...
            try:
                ...
                """).fetchall()
                _elog("db_cleanup_sizes_before",tables=[{"table":r["table_name"],"bytes":int(r["bytes"])} for r in _rows])
            except Exception as _e:
                _elog("db_cleanup_size_probe_error",error=f"{type(_e).__name__}: {_e}")

            _truncated=[]
            for _table in ("product_snapshots","macro_snapshots","visitor_sessions"):
                try:
                    _exists=_c.execute("SELECT to_regclass(%s) AS r",(f"public.{_table}",)).fetchone()
                    if _exists and _exists["r"]:
                        _c.execute(f'TRUNCATE TABLE "{_table}" RESTART IDENTITY')
                        _truncated.append(_table)
                        _elog("db_cleanup_truncate",table=_table)
                except Exception as _e:
                    _elog("db_cleanup_truncate_error",table=_table,error=f"{type(_e).__name__}: {_e}")
            ...
            try:
                while True:
                    _rows=_c.execute("""
                        WITH doomed AS (
                            SELECT ctid FROM ledger_events
                            WHERE event_type='meta_signal'
                            LIMIT 5000
                        )
                        DELETE FROM ledger_events l
                        USING doomed d
                        WHERE l.ctid=d.ctid
                        RETURNING 1
                    """).fetchall()
                    _n=len(_rows); _deleted_meta+=_n
                    if _n==0:
                        break
                    if _deleted_meta % 25000 == 0:
                        _elog("db_cleanup_progress",meta_signal_deleted=_deleted_meta)
                _elog("db_cleanup_meta_signal_done",deleted=_deleted_meta)
            except Exception as _e:
                _elog("db_cleanup_meta_signal_error",deleted=_deleted_meta,error=f"{type(_e).__name__}: {_e}")
            ...
            try:
                _rows=_c.execute("""
                    ...
                    RETURNING 1
                """).fetchall()
                _deleted_alerts=len(_rows)
                _elog("db_cleanup_old_alerts_done",deleted=_deleted_alerts)
            except Exception as _e:
                _elog("db_cleanup_old_alerts_error",error=f"{type(_e).__name__}: {_e}")

            for _table in ("ledger_events","product_alerts"):
                try:
                    _c.execute(f'VACUUM (ANALYZE) "{_table}"')
                    _elog("db_cleanup_vacuum",table=_table)
                except Exception as _e:
                    _elog("db_cleanup_vacuum_error",table=_table,error=f"{type(_e).__name__}: {_e}")
            ...
            try:
                _rows=_c.execute("""
                    ...
                    ORDER BY pg_total_relation_size(relid) DESC LIMIT 15
                """).fetchall()
                _elog("db_cleanup_sizes_after",tables=[{"table":r["table_name"],"bytes":int(r["bytes"])} for r in _rows])
            except Exception as _e:
                _elog("db_cleanup_size_probe_after_error",error=f"{type(_e).__name__}: {_e}")
            ...
            try:
                ...
                """,(_marker,_json.dumps({
                    ...
                    "meta_signal_deleted":_deleted_meta,
                    "old_alerts_deleted":_deleted_alerts
                })))
                _elog("db_cleanup_complete",marker=_marker,truncated=_truncated,
                      meta_signal_deleted=_deleted_meta,old_alerts_deleted=_deleted_alerts)
            except Exception as _e:
                _elog("db_cleanup_marker_error",error=f"{type(_e).__name__}: {_e}")
    except Exception as _e:
        _elog("db_cleanup_connection_error",error=f"{type(_e).__name__}: {_e}")

try:
    _v90_emergency_pg_cleanup()
except Exception:
    pass
