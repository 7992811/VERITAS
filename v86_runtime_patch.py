...
if _v90_os.getenv('VERITAS_STORAGE_GENERATION','').strip()=='9.0':
    ...
    with _v90_psycopg.connect(_v90_dsn,autocommit=True) as _v90_conn:
        _v90_conn.execute('CREATE SCHEMA IF NOT EXISTS veritas_v90_prod')
    _v90_dsn=_v90_make_conninfo(_v90_dsn,options='-c search_path=veritas_v90_prod')
    _v90_os.environ['DATABASE_URL']=_v90_dsn
    _v90_os.environ.pop('VERITAS_V85_TEST_DATABASE_URL',None)
    print('VERITAS_V90_DB_LEASE_ACTIVE storage_generation=9.0 schema=veritas_v90_prod',flush=True)
'''
...
_v90_sr=_v90_sr.replace("RESTORE_ID='PRE_DURABLE_STATE_20260924_111755Z'","RESTORE_ID='PRE_DURABLE_STATE_20260924_112949Z_V90PROD'")
...
_v90_existing_anchor="""        cold_start=(len(existing_accounts)==0)
        replaced=[]; skipped=[]; fee_adjustment={}
"""
_v90_existing_repl="""        _schema=c.execute('SELECT current_schema() AS s').fetchone()
        _schema_name=str(_schema['s'] if _schema else '')
        if _schema_name!='veritas_v90_prod':
            raise RuntimeError('V90_RESTORE_SCHEMA_GUARD:'+_schema_name)
        cold_start=True
        replaced=[]; skipped=[]; fee_adjustment={}
"""
