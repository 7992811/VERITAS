...
new="""            pp=dict(rr.get('trade_plan') or {})
            ...
            if gate.get('entry_mode') in ('PULLBACK','BREAKOUT') and gate.get('adjusted_stop_price'):
                pp['stop_price']=gate.get('adjusted_stop_price')
                pp['eligible']=True
                pp['reason']='entry_mode_'+str(gate.get('entry_mode')).lower()
            pp['winrate_repair_gate']=gate
            executable.append((rr,ww,pp,gate))
"""
...
new="""        desired=min(D('.25'),gate.get('max_fraction') or D('.05'))
        route_status=('ROUTED_'+str(gate.get('entry_mode'))+'_ENTRY' if gate.get('entry_mode') in ('PULLBACK','BREAKOUT') else
                      'ROUTED_MOVEMENT_'+str(gate.get('movement_state')) if gate.get('movement') else 'ROUTED_NOW_ENTRY')
