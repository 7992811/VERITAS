...
VERSION = 'veritas-max-product-v70.3-multiscale-intelligence'
try:
...
def horizon_structure_features(raw, horizon):
    ...
    floor={'BTC':0.0015,'ETH':0.0020,'NDX':0.0008,'BRENT':0.0012,'GOLD':0.0007,'MOEX':0.0010}.get(asset,0.001)
...
def features(raw, horizon, common_structure=None):
    ...
    if common_structure is not None:
        ...
        f['horizon_structure'] = (common_structure.get('horizon_structures') or {}).get(horizon) or horizon_structure_features(raw,horizon)
    else:
...
def cycle():
    ...
    phase_t0=time.time(); event_learning = refresh_event_outcomes() if EVENT_LEARNING_ENABLED else {'status':'disabled','written':0}; phase_seconds['event_learning']=time.time()-phase_t0
    phase_t0=time.time(); rule_learning = refresh_rule_stats() if pg_enabled() else {'rows': 0, 'status_changes': 0}; phase_seconds['rule_learning']=time.time()-phase_t0
    ...
    for symbol, (asset, cb_product) in ASSETS.items():
        asset_wall_t0=time.time(); asset_timings[asset]={'market_fetch':0.0,'context':0.0,'common_features':0.0,'horizons':0.0,'total':0.0}
        try:
            ...
            asset_timings[asset]['market_fetch']=time.time()-asset_phase_t0
            ...
            asset_timings[asset]['total']=time.time()-asset_wall_t0
        except Exception as e:
            asset_timings[asset]['total']=time.time()-asset_wall_t0
...
def architecture_efficiency_status():
    ...
    return {'status':'ok' if elapsed is not None else 'BUILDING',
            ...
            'principle':'Оптимизация производится только после измерения фаз. Нативные структуры горизонтов считаются локально из уже загруженных баров и не добавляют сетевых запросов.'}
