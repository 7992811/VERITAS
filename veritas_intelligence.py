...
VERSION = 'veritas-intelligence-product-v2.0'
...
MACRO_REFRESH_SECONDS = max(900, int(os.getenv('VERITAS_MACRO_REFRESH_SECONDS', '3600')))
...
def _fred_last(series_id):
    ...
    for row in reversed(rows):
        ...
        if v and v != '.':
            return {'value':float(v),'date':row.get('DATE'),'source':'FRED','series':series_id}
...
def _yahoo_chart(symbol):
    ...
    for url in urls:
        try:
            with httpx.Client(timeout=15, headers={'User-Agent':'Mozilla/5.0 VERITAS'}) as h:
                r = h.get(url, params={'range':'5d','interval':'1d','includePrePost':'false'})
                r.raise_for_status()
                j = r.json()
            ...
            t,v = vals[-1]
            return {'value':float(v),'ts':datetime.fromtimestamp(t,tz=timezone.utc).isoformat(),
                    'source':'Yahoo Finance shadow','symbol':symbol}
        except Exception as ex:
            last_err = ex
    raise RuntimeError(f'YAHOO_FAIL {symbol}: {last_err}')
...
def fetch_macro_context():
    ...
    data, errors = {}, []
    for key,series in {'ust2y':'DGS2','ust10y':'DGS10','ust30y':'DGS30','vix':'VIXCLS','fedfunds':'DFF'}.items():
        try:
            data[key] = _fred_last(series)
        except Exception as ex:
            errors.append(f'{key}:{type(ex).__name__}:{ex}')
    for key,symbol in {'nasdaq':'%5EIXIC','sp500':'%5EGSPC','dxy':'DX-Y.NYB','gold':'GC%3DF'}.items():
        try:
            data[key] = _yahoo_chart(symbol)
        except Exception as ex:
            errors.append(f'{key}:{type(ex).__name__}:{ex}')
...
def maybe_create_alert(entity_key, asset, horizon, decision, confidence, score, regime, kmatches):
    ...
    if decision in ('LONG','SHORT') and float(confidence) >= ALERT_CONFIDENCE_THRESHOLD:
        reasons.append(f'confidence_threshold:{float(confidence):.3f}')
        if severity == 'info':
            severity='medium'
    if not reasons:
        return None
...
def product_overview():
    ...
    return {'version':VERSION,'product':'VERITAS Markets','mode':'research_shadow','live_capital_execution':False,
            ...
            'macro':get_macro_context(),'macro_regime':macro_regime_summary(),'alerts':recent_alerts(20),
