...
VERSION = 'veritas-max-product-v10.0-final-intelligence-stack'
...
def refresh_event_outcomes(limit=None):
    ...
    for rr in rows:
        ...
        targets=[e['asset']] if e['asset'] in ('BTC','ETH','NDX') else ['BTC','ETH','NDX']
        ...
        for target in targets:
            symbol='BTCUSDT' if target=='BTC' else 'ETHUSDT' if target=='ETH' else 'NDX'
            try:
                with pg_connect() as c:
                ...
                if target=='NDX':
                    ...
                    specs={'4h':4,'1d':NDX_HORIZON_BARS['1d']}
                else:
...
{{"events":[{{"asset":"BTC|ETH|NDX|GLOBAL","direction":"LONG|SHORT|NEUTRAL|RISK_ON|RISK_OFF",
...
def run_event_web_scan(reason='scheduled'):
    ...
    try:
        ...
        for e in raw[:8] if isinstance(raw,list) else []:
            ...
            if asset not in ('BTC','ETH','NDX','GLOBAL'):
                continue
            direction=str(e.get('direction') or 'NEUTRAL').upper()
...
def meta_cio_board_from_summary(summary=None):
    ...
    evctx={a:event_shadow_score_learned(a) for a in ('BTC','ETH','NDX')}
    ...
    for x in summary:
        ...
        items.append({
            ...
            'meta_reliability':mr,'contradiction':contradiction,'live_influence':False
        })
...
def release_candidate_dashboard():
    return {'opportunities':opportunity_board(),'meta_performance':meta_performance_board(),
...
<div class="card span12"><div class="k">Лучшие возможности Meta-CIO</div><div id="opps" class="note">—</div></div>
...
<div class="card span4"><div class="k">VAULT / устойчивость</div><div id="vaultq" class="note">—</div></div><div class="card span4"><div class="k">Издержки</div><div id="costq" class="note">—</div></div><div class="card span4"><div class="k">Готовность сигналов</div><div id="readyq" class="note">—</div></div><div class="card span4"><div class="k">Meta-CIO: факт</div><div id="metaperf" class="note">—</div></div>
