...
VERSION = 'veritas-max-product-v15.0-six-assets'
...
ASSET_HORIZON_BARS = {
    'NDX':   {'4h':4,'1d':7,'3d':20,'7d':46},
    'MOEX':  {'4h':4,'1d':9,'3d':27,'7d':63},
    'BRENT': {'4h':4,'1d':23,'3d':69,'7d':161},
    'GOLD':  {'4h':4,'1d':23,'3d':69,'7d':161},
}
...
MOEX_FREE_ISS_DELAY_SECONDS = max(0, int(os.getenv('VERITAS_MOEX_FREE_ISS_DELAY_SECONDS','900')))
...
def all_knowledge():
    ...
    by_r = {x['rule_id']: x for x in (KNOWLEDGE_RULES + NDX_KNOWLEDGE_RULES + CROSS_ASSET_KNOWLEDGE_RULES + MANAGER_PUBLIC_RULES)}
    for x in ext_s:
        if x.get('source_id'):
            by_s[x['source_id']] = x
    for x in ext_r:
...
def discover_semantic_scholar(query):
    """Third discovery path with real abstracts."""
    if not KNOWLEDGE_SEMANTIC_FALLBACK:
        return []
    fields='title,authors,year,abstract,url,citationCount,externalIds,venue'
    with httpx.Client(timeout=25,headers={'User-Agent':'VERITAS/13 research'}) as h:
        r=h.get('https://api.semanticscholar.org/graph/v1/paper/search',
                params={'query':query,'limit':min(10,KNOWLEDGE_DISCOVERY_LIMIT),'fields':fields})
        r.raise_for_status()
        data=r.json() or {}
    out=[]
    for p in data.get('data') or []:
        ...
        out.append({'candidate_id':_candidate_id(doi,pid,title),'query':query,'title':title,
                    'authors':authors,'year':p.get('year'),'doi':doi,
                    'source_url':('https://doi.org/'+doi) if doi else url,
...
def _yahoo_research_futures_market(asset,yahoo_symbol,proxy_symbol,policy_key,source_name):
    ...
    _set_source_quality(quality)
    return {'asset':asset,'price':price,'secondary_price':price,'coinbase_price':price,
            ...
            'source_names':{'primary':source_name,'secondary':f'{proxy_symbol} proxy'}}
...
def _moex_candles_between(start_ts,end_ts):
    ...
    base='https://iss.moex.com/iss/history/engines/stock/markets/index/boards/SNDX/securities/IMOEX/candles.json'
    out=[]; start=0
    with httpx.Client(timeout=25,headers={'User-Agent':'VERITAS/15 research'}) as h:
        for _ in range(30):
...
def _moex_market():
    end=time.time(); hist=_moex_candles_between(end-50*86400,end+86400)
    if len(hist)<200:
    ...
    _set_source_quality(quality)
    return {'asset':'MOEX','price':price,'secondary_price':secondary or price,'coinbase_price':secondary or price,
...
def _fetch_history(symbol, days):
    ...
    if symbol=='MOEX':
        d=min(int(days),MOEX_BACKTEST_DAYS); return _moex_candles_between(end-d*86400,end)
...
def _event_category(e):
    ...
    mapping=[
      ...
      ('geopolitics',('war','sanction','tariff','geopolit')),
        ...
        for target in targets:
            ...
            try:
                ...
                if target in MARKET_BAR_ASSETS:
                    ...
                    specs={'4h':horizon_bars(target,'4h'),'1d':horizon_bars(target,'1d')}
                else:
...
Search the web for material market-moving developments from approximately the last {EVENT_WEB_SCAN_LOOKBACK_MINUTES} minutes relevant to BTC, ETH, Nasdaq-100, US rates/liquidity, or broad global risk.
...
"category":"rates|inflation|growth|liquidity|earnings|regulation|etf_flows|crypto_specific|geopolitics|positioning|other",
    ...
    try:
        ...
        for e in raw[:8] if isinstance(raw,list) else []:
            ...
            if category not in ('rates','inflation','growth','liquidity','earnings','regulation','etf_flows',
                                'crypto_specific','geopolitics','positioning','other'):
                category='other'
            item={
...
def meta_cio_board_from_summary(summary=None):
    ...
    items=[]
    for x in summary:
        asset=x.get('asset'); horizon=x.get('horizon'); dec=x.get('decision')
        ...
        horizon_agreement=len([p for p in by_asset.get(asset,[]) if p.get('decision')==dec and dec in ('LONG','SHORT')])
...
def meta_performance_board():
    ...
    if not pg_enabled(): return {'status':'unavailable','items':[]}
    with pg_connect() as c:
        rows=c.execute("""SELECT m.asset,m.horizon,m.payload mp,o.payload op
                          FROM ledger_events m JOIN ledger_events o
                          ON o.entity_key=m.entity_key AND o.event_type='outcome'
                          WHERE m.event_type='meta_signal'""").fetchall()
    b={}
    for r in rows:
...
def _no_trade_move_threshold(horizon):
    return {'4h':NO_TRADE_MISSED_MOVE_4H,'1d':NO_TRADE_MISSED_MOVE_1D,
...
def correlation_matrix():
    with correlation_cache_lock:
        if correlation_cache.get('value') and time.time()-float(correlation_cache.get('at') or 0)<21600: return correlation_cache['value']
    try:
        series={'BTC':_daily_crypto_returns('BTCUSDT',PORTFOLIO_CORR_LOOKBACK_DAYS),'ETH':_daily_crypto_returns('ETHUSDT',PORTFOLIO_CORR_LOOKBACK_DAYS),'NDX':_daily_ndx_returns(PORTFOLIO_CORR_LOOKBACK_DAYS)}
        assets=list(series); matrix={a:{b:(1.0 if a==b else None) for b in assets} for a in assets}
        for i,a in enumerate(assets):
            for b in assets[i+1:]:
                dates=sorted(set(series[a]).intersection(series[b])); c=_pearson([series[a][d] for d in dates],[series[b][d] for d in dates]); matrix[a][b]=matrix[b][a]=c
        out={'status':'ok','lookback_days':PORTFOLIO_CORR_LOOKBACK_DAYS,'matrix':matrix,'note':'Daily-return shadow correlation; NDX uses trading days only.'}
    except Exception as ex: out={'status':'error','error':f'{type(ex).__name__}: {ex}','matrix':{}}
    with correlation_cache_lock: correlation_cache['at']=time.time(); correlation_cache['value']=out
    return out
...
def asset_thesis_board():
    meta=meta_cio_board_from_summary()
    hw={'4h':0.75,'1d':1.0,'3d':1.15,'7d':1.0}
    ...
    for asset,rows in by.items():
        ...
        for x in rows:
            d=x.get('meta_decision'); sign=1 if d=='LONG' else -1 if d=='SHORT' else 0
...
def model_card():
    return {'version':VERSION,'generated_at':now(),
      'purpose':'Research decision-support for BTC, ETH and Nasdaq-100; no automated live execution.',
      'assets':list(DISPLAY_ASSETS),'horizons':['4h','1d','3d','7d'],
...
def release_candidate_dashboard():
    return {'opportunities':opportunity_board(),'asset_thesis':asset_thesis_board(),
            'meta_performance':meta_performance_board(),'policy_lab':policy_counterfactual_board(),
            ...
            'portfolio_allocator':portfolio_allocator(),'scenarios':scenario_board(),
...
<div class="card span4"><div class="k">VAULT / устойчивость</div><div id="vaultq" class="note">—</div></div><div class="card span4"><div class="k">Издержки</div><div id="costq" class="note">—</div></div><div class="card span4"><div class="k">Готовность сигналов</div><div id="readyq" class="note">—</div></div><div class="card span4"><div class="k">Policy Lab</div><div id="policy" class="note">—</div></div>
...
function renderMatrix(a){a=Array.isArray(a)?a:[];const map={};a.forEach(x=>{if(x&&x.asset&&x.horizon)map[x.asset+'|'+x.horizon]=x});document.getElementById('matrix').innerHTML=assets.map(asset=>`<tr><td><span class="asset-name">${asset}</span></td>${tfOrder.map(tf=>{const x=map[asset+'|'+tf];if(!x)return'<td><span class="stamp">нет данных</span></td>';return`<td><button class="signal-cell" onclick="showDetail('${asset}','${tf}')" title="${tierText(x)}"><i class="dot ${dotClass(x)}"></i><div class="strength">${pct(x.confidence)}</div><div class="cal">${x.calibrated_probability==null?'':'P '+pct(x.calibrated_probability)}</div></button></td>`}).join('')}</tr>`).join('');const expected=assets.length*tfOrder.length,loaded=a.filter(x=>assets.includes(x.asset)&&tfOrder.includes(x.horizon)).length,missing=expected-loaded;document.getElementById('matrixstatus').textContent=missing<=0?`${loaded}/${expected} сигналов загружены`:`${loaded}/${expected} · отсутствует ${missing} ячеек`;document.getElementById('superstrip').innerHTML=tfOrder.map(tf=>{const xs=a.filter(x=>x.horizon===tf&&(x.signal_tier==='SUPER_LONG'||x.signal_tier==='SUPER_SHORT'));return`<div class="superbox"><div class="tf">${tf}</div><div class="superline">${xs.length?xs.map(x=>`<span class="superasset"><i class="dot ${dotClass(x)}"></i>${x.asset}</span>`).join(''):'<span class="stamp">нет усиленного сигнала</span>'}</div></div>`}).join('')}
...
def main():
    ...
    emit('product_ready', dashboard='/app', api='/api/v1/overview', history_api='/api/v1/history',
         ...
         model_card_api='/api/v1/model-card', knowledge_import_api='/admin/knowledge/import',
