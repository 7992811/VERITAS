...
VERSION = 'veritas-max-product-v22.1-adaptive-expert-replay-learning'
...
def structure_analog_board(limit=1200):
    ...
    with pg_connect() as c:
        rows=c.execute("""SELECT d.asset,d.horizon,d.payload dp,o.payload op
                          FROM ledger_events d JOIN ledger_events o ON o.entity_key=d.entity_key AND o.event_type='outcome'
                          WHERE d.event_type='decision' ORDER BY d.event_ts DESC LIMIT %s""",(int(limit),)).fetchall()
    b={}
    for r in rows:
...
def _active_setup(asset,horizon):
    if not pg_enabled(): return None
    with pg_connect() as c:
        r=c.execute("SELECT * FROM trade_setups WHERE asset=%s AND horizon=%s AND status='ACTIVE' ORDER BY created_at DESC LIMIT 1",(asset,horizon)).fetchone()
...
def _recent_stop_count(asset,horizon,hours=None):
    if not pg_enabled(): return 0
    hours=int(hours or REENTRY_STOP_WINDOW_HOURS)
    with pg_connect() as c:
        r=c.execute("""SELECT COUNT(*) n FROM product_alerts
                       WHERE asset=%s AND horizon=%s AND alert_type='STOP'
                         AND created_at >= NOW()-(%s || ' hours')::interval""",(asset,horizon,str(hours))).fetchone()
    return int((r or {}).get('n') or 0)


def manage_trade_alerts(summary):
    """Create ENTRY/TACTICAL_ENTRY and manage EXIT/STOP/INVALIDATION in shadow mode. Robot-consumable, no order execution."""
    if not TRADE_ALERTS_ENABLED or not pg_enabled(): return []
    by={(x.get('asset'),x.get('horizon')):x for x in summary}; out=[]
    # First manage existing shadow setups.
    for (asset,h),x in by.items():
        setup=_active_setup(asset,h)
        ...
        if terminal:
            ...
            out.append(_insert_trade_alert(asset,h,terminal,'high',payload))
            with pg_connect() as c: c.execute("UPDATE trade_setups SET status=%s,updated_at=%s,payload=%s::jsonb WHERE setup_id=%s",(terminal,now(),json.dumps(payload,ensure_ascii=False),setup['setup_id']))
    # Then create new entries where there is no active setup.
    for (asset,h),x in by.items():
        if _active_setup(asset,h): continue
        ...
        # NO_TRADE on higher TF is treated as non-confirmation, not opposition; still disclose it.
        ...
        recent_stops=_recent_stop_count(asset,h)
        # Expert policy: two stop-outs in a noisy range require a new confirmed breakout after volatility cools.
        if recent_stops>=REENTRY_RANGE_STOP_LIMIT:
            rg=str(x.get('regime') or '')
            st=(x.get('intraday_structure') or {})
        ...
        out.append(_insert_trade_alert(asset,h,atype,sev,payload))
        with pg_connect() as c:
