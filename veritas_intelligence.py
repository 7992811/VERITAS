...
VERSION = 'veritas-max-product-v19.0-regime-risk-budget-autonomy'
...
def dynamic_risk_budget(allocation=None,risk=None,transition_board=None):
    """Regime/correlation/tail-risk aware shadow risk budget. Never increases above current allocator weights."""
    alloc=allocation or portfolio_allocator()
    ...
    if cvar is None:
        ...
        try:
            ...
            cvar_value=None
        if cvar_value is None or not math.isfinite(cvar_value):
            portfolio_mult=0.70
        else:
            portfolio_mult=1.0 if cvar_value<=PORTFOLIO_TARGET_DAILY_CVAR95 else max(PORTFOLIO_MIN_RISK_MULTIPLIER,PORTFOLIO_TARGET_DAILY_CVAR95/max(cvar_value,1e-9))
    trans_by={}
    for x in (transitions.get('items') or []):
        a=x.get('asset'); lvl=str(x.get('transition_risk') or 'BUILDING').upper()
        rank={'LOW':0,'BUILDING':1,'MEDIUM':2,'HIGH':3}.get(lvl,1)
        prev=trans_by.get(a)
    ...
    for p in positions:
        a=p['asset']; base=max(0.0,float(p.get('weight') or 0))
        lvl=(trans_by.get(a) or (1,'BUILDING'))[1]
        trans_mult=_risk_level_multiplier(lvl)
        ...
        corr_mult=0.65 if avg_abs_corr>=0.75 else 0.80 if avg_abs_corr>=0.60 else 1.0
        dp=_latest_decision_payload(a,p.get('horizon')) or {}
        ...
        combined=max(0.0,min(1.0,portfolio_mult*trans_mult*tail_mult*corr_mult))
        research_budget=base*combined
        deployable_budget=research_budget*source_mult
        cls='crypto' if a in CRYPTO_ASSETS else 'equity_index' if a in EQUITY_INDEX_ASSETS else 'commodity'
        class_budget[cls]+=research_budget
        rows.append({'asset':a,'horizon':p.get('horizon'),'decision':p.get('decision'),'grade':p.get('grade'),
                     'allocator_weight':base,'research_risk_budget':research_budget,
                     'deployable_risk_budget':deployable_budget,'portfolio_multiplier':portfolio_mult,
                     'transition_risk':lvl,'transition_multiplier':trans_mult,'tail_share_abs':share_abs,
                     'tail_multiplier':tail_mult,'avg_abs_corr_to_held':avg_abs_corr,'correlation_multiplier':corr_mult,
                     'execution_eligible':execution_eligible,'source_multiplier':source_mult})
    gross_base=sum(float(x.get('weight') or 0) for x in positions)
    gross_budget=sum(float(x.get('research_risk_budget') or 0) for x in rows)
    deployable=sum(float(x.get('deployable_risk_budget') or 0) for x in rows)
    if not rows: posture='NO_RISK'
    elif portfolio_mult<0.60: posture='DEFENSIVE'
    ...
    return {'status':'ok' if rows else 'no_positions','mode':'shadow','risk_posture':posture,
            'target_daily_cvar95':PORTFOLIO_TARGET_DAILY_CVAR95,'measured_cvar95':cvar_value,
            'portfolio_multiplier':portfolio_mult,'gross_allocator_weight':gross_base,
            'gross_research_risk_budget':gross_budget,'gross_deployable_risk_budget':deployable,
            'cash_or_unallocated_fraction':max(0.0,1.0-gross_budget),
            'class_budgets':class_budget,'asset_budgets':rows,
            'live_execution':False,'decision_gate':'SHADOW_ONLY',
            'note':'Governance risk budget: current allocator weights are ceilings. Regime transitions, tail concentration and correlation can only reduce risk; no automatic leverage increase or live trading.'}
...
def portfolio_meta_cio(allocation=None,risk=None,risk_budget=None):
    ...
    return {'status':'ok' if positions else 'no_positions','mode':'shadow','portfolio_bias':bias,
            'risk_posture':rb.get('risk_posture'),'gross_weight':gross,'net_weight':net,'positions':positions,
...
def release_candidate_dashboard():
    ...
    risk=portfolio_tail_risk(alloc); transitions=regime_transition_board()
    rb=dynamic_risk_budget(alloc,risk,transitions); pm=portfolio_meta_cio(alloc,risk,rb)
    return {'opportunities':opp,'asset_thesis':asset_thesis_board(),
            ...
            'portfolio_allocator':alloc,'portfolio_risk':risk,'dynamic_risk_budget':rb,'portfolio_meta_cio':pm,'causal_drivers':causal_driver_board(),
...
def compute_product_overview():
    ...
    transitions=regime_transition_board()
    rb=dynamic_risk_budget(alloc,risk,transitions)
    ...
    out={'version':VERSION,'product':'VERITAS Markets','mode':'research_shadow','live_capital_execution':False,
            ...
            'portfolio_risk':risk,'dynamic_risk_budget':rb,'portfolio_meta_cio':pm,
            'correlations':corr,'autonomy':autonomy_status(),'horizon_integrity':horizon_integrity_status(),
    ...
    def do_GET(self):
        try:
            if self.path == '/app' or self.path.startswith('/app?'):
                ...
                self.reply({'status':'ok','version':VERSION,'ts':now(),'runtime_id':RUNTIME_ID})
...
def main():
    ...
    emit('service_start', db_path=DB_PATH, interval=INTERVAL, postgres=pg_boot, knowledge_pg=pg_knowledge,
         runtime_id=SERVICE_RUNTIME_ID, always_on_confirmed=PRODUCTION_ALWAYS_ON,
