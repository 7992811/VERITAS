...
SUPPORTED_RULE_FIELDS = {'ret_h','ret_4h','ret_24h','ret_72h','ret_168h','trend','momentum','rv','volume_ratio','taker_buy_share','source_divergence','funding','basis','oi_change_24h','taker_buy_sell_ratio','global_long_short_ratio','intraday_structure_score','relative_volume','near_ath','price_discovery','breakout_hold','session_efficiency','session_persistence','expected_move_pct'}
SUPPORTED_RULE_OPS = {'>','>=','<','<=','=='}
DISCOVERY_QUERIES = [
...
def generic_structure_features(raw):
    ...
    hold=1.0 if breakout.get('found') and breakout.get('held') else 0.15 if breakout.get('found') else 0.40
    ...
    invalidation=breakout.get('invalidation') if breakout.get('found') else (min(l[-4:]) if direction=='LONG' else max(h[-4:]))
...
def merge_trend_and_structure(trend, structure):
    ...
    if st.get('direction') in ('LONG','SHORT'):
        ...
        if st.get('lifecycle') in ('CONFIRMATION','EXTENSION') and not st.get('false_breakout'):
...
def trend_onset_features(raw):
    ...
    floors={'BTC':0.0015,'ETH':0.0020,'NDX':0.0010,'BRENT':0.0015,'GOLD':0.0008,'MOEX':0.0012}
...
def features(raw, horizon, common_structure=None):
    ...
    if common_structure is not None:
        ...
        f['trend_impulse'] = common_structure.get('trend_impulse') or {}
    else:
...
def classify_signal_tier(asset,decision,confidence,challenger,effective_evidence,source_gate,time_gate,calibration=None,trend_impulse=None):
    ...
    return ('SUPER_'+decision) if (super_cal or super_cons or market_structure_super) else decision
...
def cycle():
    ...
    outcomes_seconds=time.time()-phase_t0
    ...
    analog_seconds=time.time()-phase_t0
                ...
                if pg_enabled():
                    try:
                        pg_event('decision', entity_key, {
                            ...
                            'features': f, 'derivatives': deriv,'event_shadow':event_shadow,'v70_pretrade':v70_pretrade,
                            'causal_shadow':causal_shadow,
    ...
    state = {'status': status, 'at': now(), 'version': VERSION, 'decisions_written': made,
             ...
             'telemetry':telemetry,'knowledge': knowledge_summary()}
    with lock:
...
def technical_trade_plan(asset,horizon,f,research_decision,signal_tier,analog=None):
    ...
    pullback=st.get('pullback_anchor'); breakout=st.get('breakout_level'); day_extreme=st.get('session_low') if direction=='LONG' else st.get('session_high')
    ...
    stop_dist=abs(p-stop)/p if p else 999.0; exp=float(est.get('expected_move_pct') or 0.0); ratio=exp/stop_dist if stop_dist>1e-12 else 999.0
    # Full size is reserved for strong, volume-confirmed impulse breakouts; otherwise stage the entry.
    strong_break=bool(st.get('breakout_hold') and st.get('volume_confirmed') and float(st.get('score') or 0)>=STRUCTURE_STRONG_SCORE and str(ti.get('phase') or '') in ('TREND_DAY','IMPULSE_TREND'))
...
def manage_trade_alerts(summary):
    ...
    for (asset,h),x in by.items():
        ...
        if recent_stops>=REENTRY_RANGE_STOP_LIMIT:
            ...
            if 'RANGE' in rg or 'HIGH_VOL' in rg:
                if not (str(st.get('lifecycle') or '') in ('CONFIRMATION','EXTENSION') and st.get('breakout_hold') and 'HIGH_VOL' not in rg):
                    continue
        atype='TACTICAL_ENTRY' if counter else 'ENTRY'
        ...
        payload={'schema_version':TRADE_ALERT_SCHEMA_VERSION,'setup_id':setup_id,'trigger_ts':now(),'asset':asset,'horizon':h,
                 ...
                 'comment':('Тактический вход против старшего таймфрейма; минимальный ход нормирован на волатильность и масштаб движения старшего ТФ.' if counter else 'Вход подтвержден структурой текущего таймфрейма.'),
                 'robot_eligible':False,'execution_mode':'SHADOW_ONLY'}
        ...
        sev='medium' if plan.get('late_entry') else ('high' if x.get('signal_tier') in ('SUPER_LONG','SUPER_SHORT') else 'medium')
...
def dynamic_risk_budget(allocation=None,risk=None,transition_board=None,experience=None):
    ...
    rows=[]; class_budget={'crypto':0.0,'equity_index':0.0,'commodity':0.0}
    for p in positions:
        ...
        cls='crypto' if a in CRYPTO_ASSETS else 'equity_index' if a in EQUITY_INDEX_ASSETS else 'commodity'; class_budget[cls]+=research_budget
...
def signal_capacity_status():
    ...
    return {'primary_signal_cells':len(DISPLAY_ASSETS)*len(HORIZONS),'assets':len(DISPLAY_ASSETS),'horizons':len(HORIZONS),
            'agents':len(BASE_WEIGHTS),'supported_rule_fields':len(SUPPORTED_RULE_FIELDS),
            'avg_live_state_fields':round(sum(scalar)/len(scalar),1) if scalar else 0,
            'max_live_state_fields':max(scalar) if scalar else 0,
            'explanation':'30 is 6 assets × 5 horizons, not the number of model parameters. Depth is expanded inside each cell.'}
...
def v701_learning_bundle(limit=None):
    ...
    for r in rows:
        ...
        cf_base=sr if sr is not None else 0.0
        cf_gate=cf_base
        if sr is not None and g:
            ...
            cf_gate=(sr*mult) if allow else 0.0
            delta=cf_gate-sr; gate_delta.append(delta); gate_cf.append(cf_gate); gate_base.append(sr)
            if not allow:
                veto_n+=1
                z=gate_type_stats.setdefault(gc,{'n':0,'good':0,'bad':0,'net_value':0.0}); z['n']+=1; z['net_value']+=delta
                if sr<0: avoided_n+=1; avoided_loss+=-sr; z['good']+=1
                elif sr>0: blocked_n+=1; blocked_gain+=sr; z['bad']+=1
    ...
    measurable=gate_n>=30 and veto_n>=15 and mean_abs and mean_abs>1e-12
    ...
    for gc,z in gate_type_stats.items():
        z['precision']=z['good']/max(1,z['good']+z['bad']); z['net_value']=round(z['net_value'],6)
    layer_items=[]
    if gate_delta:
        layer_items.append({'layer':'V70_GATE','n':len(gate_delta),'avg_incremental_return':sum(gate_delta)/len(gate_delta),'total_incremental_return':sum(gate_delta),'status':'MEASURABLE' if len(gate_delta)>=30 else 'BUILDING'})
    ...
    out={'status':'ok','method':'independent completed episodes; repeated five-minute snapshots are collapsed',
         ...
         'v70_gate':{'status':'MEASURABLE' if measurable else 'BUILDING_SAMPLE','directional_outcomes_with_v70':gate_n,
                     'gate_counts':gate_counts,'veto_outcomes':veto_n,'veto_precision':veto_precision,
         ...
         'incremental_learning':{'status':'MEASURABLE' if measurable else 'BUILDING_SAMPLE','learning_index_3':idx,'baseline_index':100,
                                 ...
                                 'mean_abs_market_move':mean_abs,'required_directional_outcomes':30,'required_veto_outcomes':15,
                                 'definition':'100 = то же самое решение без v70. Индекс растёт только если теневой v70 улучшает средний результат относительно масштаба фактических движений на той же matched-выборке.'},
         ...
         'layer_attribution':{'status':'ok','items':layer_items,
                              'not_yet_attributable':['KNOWLEDGE','CAUSAL','EVENT','CALIBRATION','REGIME'] if not rows else ['KNOWLEDGE','CAUSAL','EVENT','CALIBRATION','REGIME'],
...
def v70_pretrade_shadow(asset,horizon,research_decision,confidence,calibration,agents,
    ...
    try:
        return V70.pretrade_gate({
          ...
          'trend_impulse':f.get('trend_impulse') or {},'intraday_structure':f.get('intraday_structure') or {},
...
def architecture_efficiency_status():
    ...
    return {'status':'ok' if elapsed is not None else 'BUILDING',
            ...
            'target_cycle_seconds':30.0,
            'target_status':'ON_TARGET' if elapsed is not None and float(elapsed)<=30 else 'IMPROVING' if elapsed is not None else 'BUILDING',
            'principle':'Повторно используем только детерминированные признаки одного рыночного снимка; данные, зависящие от горизонта, остаются отдельными.'}
...
def fast_product_overview():
    ...
    return {
        ...
        'investor_asset_view':investor_view,'decision_effectiveness':learning70.get('effectiveness',{}),
...
def main():
    ...
    emit('product_ready', dashboard='/app', api='/api/v1/overview', history_api='/api/v1/history',
         ...
         expert_replay_api='/api/v1/expert-replay', expert_policy_api='/api/v1/expert-policy', decision_journal_api='/api/v1/decision-journal', structure_analogs_api='/api/v1/structure-analogs', tradeability_api='/api/v1/tradeability', large_move_capture_api='/api/v1/large-move-capture', intelligence_scorecard_api='/api/v1/intelligence-scorecard', trade_lifecycle_api='/api/v1/trade-lifecycle', missed_trends_api='/api/v1/missed-trends', early_entry_efficiency_api='/api/v1/early-entry-efficiency', event_reaction_api='/api/v1/event-reaction', regime_router_api='/api/v1/regime-router', error_attribution_api='/api/v1/error-attribution', research_agenda_api='/api/v1/research-agenda', experiments_api='/api/v1/experiments', v27_quality_api='/api/v1/v27-quality', v70_api='/api/v1/v70', architecture_efficiency_api='/api/v1/architecture-efficiency', v70_effectiveness_api='/api/v1/v70-effectiveness', v70_learning_api='/api/v1/v70-learning', investor_asset_view_api='/api/v1/investor-asset-view',
