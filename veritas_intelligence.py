...
VERSION = 'veritas-max-product-v1.1-ndx-quality'
...
def _source_row(name, asset_class, role, observed_ts=None, documented_delay_sec=None,
    ...
    if status is None:
        ...
        elif documented_delay_sec and documented_delay_sec>=600: status='DELAYED_CONTEXT'
        elif age>max(180,(documented_delay_sec or 0)+300): status='STALE_OR_CLOSED'
        else: status='OK'
    return {'source':name,'provider':provider or name,'asset_class':asset_class,'role':role,
            ...
            'documented_delay_seconds':documented_delay_sec,'status':status,'detail':detail}
...
def data_quality_snapshot():
    ...
    return {'updated_at':updated,'rows':sorted(rows,key=lambda x:(x.get('asset_class') or '',x.get('source') or '')),
        ...
        try:
            with httpx.Client(timeout=20,headers={'User-Agent':'Mozilla/5.0 VERITAS'}) as h:
                r=h.get(f'https://{host}/v8/finance/chart/{symbol}',
                        params={'range':range_,'interval':interval,'includePrePost':'true' if prepost else 'false','events':'div,splits'})
            ...
            for i,t in enumerate(ts):
                ...
                rows.append({'ts':int(t),'open':float(o if o is not None else c),'high':float(hi if hi is not None else c),
                             'low':float(lo if lo is not None else c),'close':float(c),'volume':float(vol)})
            if rows: return rows,res.get('meta') or {}
...
def pg_agent_performance():
    """Durable agent learning reconstructed from immutable decision/outcome payloads."""
    if not pg_enabled():
...
def adaptive_multiplier(agent, asset, horizon, perf, regime='*'):
    ...
    if not row or row.get('n',0) < AGENT_ADAPT_MIN_N:
        return 1.0
    hr=float(row.get('bayes_hit_rate') or row.get('hit_rate') or 0.5)
    ar=float(row.get('avg_signed_return') or 0.0)
    edge=clip((hr-0.5)*2,-0.30,0.30)
    ret_bonus=clip(ar*8,-0.05,0.05)
    return clip(1.0 + 0.45*edge + ret_bonus, 0.85, 1.15)


def pg_calibration_map():
    """Empirical correctness calibration by asset/horizon/confidence bucket."""
...
def calibrated_direction_probability(asset,horizon,confidence,cal_rows):
    ...
    return {'status':'empirical','n':row['n'],
            'probability_correct':round(float(row['posterior_hit_rate']),4),
            'raw_confidence':float(confidence),'bucket':bucket,
            'brier_raw':row.get('brier_raw')}
...
def shadow_position_sizing(decision, cal, f):
    ...
    p=float(cal['probability_correct'])
    ...
    return {'status':'shadow','fraction_of_capital':round(frac,4),
            'method':'quarter-kelly x volatility scale, capped 10%',
            'live_execution':False}




def validated_knowledge_adjustment(kmatches, asset, horizon):
    """Strict, capped CIO contribution. Disabled by default and requires live + OOS evidence."""
    if not KNOWLEDGE_CIO_ENABLED or not pg_enabled() or not kmatches:
        return {'enabled':False,'score':0.0,'rules':[]}
    ids=[x.get('rule_id') for x in kmatches if x.get('action') in ('LONG','SHORT') and x.get('rule_id')]
    if not ids: return {'enabled':True,'score':0.0,'rules':[]}
    approved=[]; total=0.0
    with pg_connect() as c:
        for rid in ids:
            ...
            oos=c.execute("""SELECT n,hit_rate,avg_signed_return FROM knowledge_backtest_oos_stats
            ...
            contrib=sign*min(0.0125,max(0.0025,float(rr['prior_weight'] or 0)*0.05))
            total+=contrib
            approved.append({'rule_id':rid,'action':rr['action'],'contribution':contrib,
                             'live_n':live['n'],'live_hit':live['hit_rate'],
                             'oos_n':oos['n'],'oos_hit':oos['hit_rate']})
    return {'enabled':True,'score':clip(total,-0.05,0.05),'rules':approved}
...
def committee(views, asset, horizon, perf, regime='*', knowledge_adjustment=0.0):
    ...
    decision = 'NO_TRADE' if KILL_SWITCH or veto or abs(x) < MIN_DIRECTIONAL_SCORE else ('LONG' if x > 0 else 'SHORT')
...
def evaluate_outcomes():
    ...
    for r in rows:
        ...
        try:
            ...
            if r['asset']!='NDX' and len(k)<hours:
                continue
            if not k:
                continue
    ...
    for symbol, (asset, cb_product) in ASSETS.items():
        try:
            ...
            for horizon in HORIZONS:
                ...
                knowledge_adjustment = validated_knowledge_adjustment(kmatches,asset,horizon)
                ...
                with db() as c:
                    ...
                    dcur = c.execute('INSERT INTO decisions(state_id,decision,confidence,sizing,synthesis,model_version,created_at) VALUES(?,?,?,?,?,?,?)',
                              (sid, dec, conf, size, json.dumps({'committee_score': score, 'weights': used_weights,
                               ...
                               'knowledge_cio_adjustment': knowledge_adjustment,
...
def run_bootstrap_backtest(reason='manual'):
    ...
    details={'reason':reason,'assets':{},'method':'mixed_asset_1h',
             'cost_bps_roundtrip':BACKTEST_COST_BPS,'oos_share':BACKTEST_OOS_SHARE,
             'warning':'Research backtest. BTC/ETH use Binance spot 1h; NDX uses up to 729d Yahoo 1h index history. Costs assumed; slippage not independently observed.'}
    ...
    try:
        ...
        rules=_historical_rules(); tested=len(rules); buckets={}; split_buckets={}
        cost=BACKTEST_COST_BPS/10000.0
        for symbol,(asset,_) in ASSETS.items():
            rows=_fetch_history(symbol,NDX_BACKTEST_DAYS if asset=='NDX' else BACKTEST_DAYS); details['assets'][asset]={'bars':len(rows),'history_days_target':NDX_BACKTEST_DAYS if asset=='NDX' else BACKTEST_DAYS}
            ...
            for idx in range(240,stop_idx,BACKTEST_SAMPLE_STEP_HOURS):
                ...
                raw=_raw_from_history(rows,idx); raw['asset']=asset; raw['source_gate_pass']=True; raw['market_open']=True; entry=float(raw['price'])
                for horizon,hh in HORIZONS.items():
                    f=features(raw,horizon)
                    if asset=='NDX':
                        target_ms=int(rows[idx][6])+hh*3600*1000
                        future=[]
                        for j in range(idx+1,len(rows)):
                            future.append(rows[j])
                            if int(rows[j][6])>=target_ms:
                                break
                        if not future or int(future[-1][6])<target_ms:
                            continue
                    else:
                        future=rows[idx+1:idx+hh+1]
                    ...
                    for rr in rules:
                        ...
                        for store_key,store in ((key,buckets),((rr['rule_id'],asset,horizon,rr['action'],sample),split_buckets)):
                            b=store.setdefault(store_key,{'n':0,'hits':0,'signed':[],'mfe':[],'mae':[],
                                                          'start':f['observed_at'],'end':f['observed_at']})
                            gross=fr if rr['action']=='LONG' else -fr
                            sr=gross-cost
                            b['n']+=1; b['hits']+=1 if sr>0 else 0; b['signed'].append(sr)
                            if rr['action']=='LONG':
                                b['mfe'].append(mfe_long); b['mae'].append(mae_long)
                            else:
                                b['mfe'].append(-mae_long if mae_long is not None else None)
                                b['mae'].append(-mfe_long if mfe_long is not None else None)
                            b['end']=f['observed_at']
                        observations+=1
        method=f'mixed_1h_crypto{BACKTEST_DAYS}d_ndx{NDX_BACKTEST_DAYS}d_step{BACKTEST_SAMPLE_STEP_HOURS}h_cost{BACKTEST_COST_BPS:g}bps'
        with pg_connect() as c:
            for (rid,asset,horizon,action),b in buckets.items():
                n=b['n']; vals=[x for x in b['signed'] if x is not None]
                mfes=[x for x in b['mfe'] if x is not None]; maes=[x for x in b['mae'] if x is not None]
            ...
            for (rid,asset,horizon,action,sample),b in split_buckets.items():
                ...
                c.execute("""INSERT INTO knowledge_backtest_oos_stats(rule_id,asset,horizon,action,sample,n,hits,hit_rate,
                    ...
                    (rid,asset,horizon,action,sample,n,b['hits'],b['hits']/n if n else None,
                     sum(vals)/len(vals) if vals else None,sum(mfes)/len(mfes) if mfes else None,
                     sum(maes)/len(maes) if maes else None,b['start'],b['end'],now()))
        ...
        state={'status':'ok','run_id':run_id,'rules_tested':tested,'observations':observations,
               'bucket_count':len(buckets),'days':BACKTEST_DAYS,'cost_bps':BACKTEST_COST_BPS,'oos_share':BACKTEST_OOS_SHARE}
    ...
    if pg_enabled():
        try:
            with pg_connect() as c:
                ...
                out['oos_top']=[dict(r) for r in c.execute("SELECT rule_id,asset,horizon,action,sample,n,hit_rate,avg_signed_return,avg_mfe,avg_mae,period_start,period_end FROM knowledge_backtest_oos_stats WHERE sample='OOS' AND n>=20 ORDER BY hit_rate DESC NULLS LAST,n DESC LIMIT 40").fetchall()]
...
def _backtest_due():
    ...
    try:
        ...
        with pg_connect() as c:
            r=c.execute("SELECT finished_at,rules_tested FROM backtest_runs WHERE status='ok' ORDER BY finished_at DESC NULLS LAST LIMIT 1").fetchone()
...
def _fred_last(series_id):
    ...
    for row in reversed(rows):
        ...
        if v and v != '.':
            return {'value':float(v),'date':row.get('DATE'),'source':'FRED H.15','series':series_id,'documented_delay_sec':86400,'freshness_class':'DAILY_REFERENCE'}
    raise RuntimeError(f'FRED_NO_VALUE {series_id}')


def _yahoo_chart(symbol,policy_key=None):
    rows,meta=_yahoo_series(symbol,'1d','1m',True)
    last=rows[-1]; current=float(last['close'])
    ...
    delay=(DATA_SOURCE_POLICY.get(policy_key or '',{}) or {}).get('documented_delay_sec')
    return {'value':current,'ts':observed,'ret_1d':ret_1d,'ret_window':ret_window,
            'source':'Yahoo Finance shadow','symbol':symbol,'documented_delay_sec':delay,
            'age_seconds':_age_seconds(observed),'meta_exchange':meta.get('exchangeName') or meta.get('fullExchangeName')}
...
def macro_refresh_loop():
    ...
    while True:
        ...
        with macro_lock:
            macro_state.clear()
            macro_state.update(x)
        emit('macro_refresh', status=x.get('status'), fields=len(x.get('data',{})),
             errors=len(x.get('errors',[])), decision_influence=False)
...
def maybe_create_alert(entity_key, asset, horizon, decision, confidence, score, regime, kmatches):
    ...
    if prev:
        ...
        if abs(float(confidence)-pc) >= ALERT_MIN_CHANGE:
            ...
            if severity != 'high':
                severity='medium'
    if decision in ('LONG','SHORT') and float(confidence) >= ALERT_CONFIDENCE_THRESHOLD:
        prev_conf = float(prev['payload'].get('confidence') or 0) if prev else 0.0
        if prev_conf < ALERT_CONFIDENCE_THRESHOLD:
