...
VERSION = 'veritas-intelligence-v1.4.0'
                ...
                with db() as c:
                    ...
                    for km in kmatches:
                        c.execute('INSERT OR IGNORE INTO knowledge_matches(state_id,rule_id,action,shadow_score,matched_at) VALUES(?,?,?,?,?)',
                                  (sid,km['rule_id'],km['action'],km['shadow_score'],now()))
                    ...
                    c.execute('INSERT INTO decisions(state_id,decision,confidence,sizing,synthesis,model_version,created_at) VALUES(?,?,?,?,?,?,?)',
                              (sid, dec, conf, size, json.dumps({'committee_score': score, 'weights': used_weights,
                               ...
                               'gates': {'scope': True, 'metric': True, 'source': True, 'time': True}}), VERSION, now()))
                made += 1
