...
VERSION = 'veritas-max-product-v25.0-release-candidate'
...
MULTILINGUAL_DISCOVERY_QUERIES = list(dict.fromkeys(MULTILINGUAL_DISCOVERY_QUERIES + MULTILINGUAL_STRUCTURE_QUERIES))
...
def cycle():
        ...
        try:
            ...
            for horizon in HORIZONS:
                ...
                if pg_enabled():
                    try:
                        pg_event('decision', entity_key, {
                            ...
                            'execution_eligibility':execution_gate,'trade_plan':trade_plan,
                ...
                z = {'asset': asset, 'horizon': horizon, 'decision': dec,
                     ...
                     'intraday_structure':f.get('intraday_structure') or {},'trade_plan':trade_plan,
    ...
    state = {'status': status, 'at': now(), 'version': VERSION, 'decisions_written': made,
             'outcomes_written': outcomes, 'summary': summary, 'trade_alerts_written':len(trade_alerts), 'meta_cio':meta_cio,'meta_alerts_written':meta_alerts,
    ...
    emit('cycle_complete', decisions_written=made, outcomes_written=outcomes, status=status,
         ...
         trade_alert_seconds=round(trade_alert_seconds,3))
    if pg_enabled():
...
def technical_trade_plan(asset,horizon,f,research_decision,signal_tier,analog=None):
    ...
    return {'eligible':eligible,'reason':'ok' if eligible else ('invalidated' if invalid else 'expected_move_too_small_vs_stop'),
            ...
            'initial_position_fraction':entry_fraction,'scaling_policy':scaling,
...
def lightweight_opportunity_board(summary=None):
    ...
    out=[]
    for x in rows:
        d=x.get('research_decision') or x.get('decision')
        if d not in ('LONG','SHORT'): continue
        tp=x.get('trade_plan') or {}; conf=float(x.get('confidence') or 0.0)
        score=100*conf + (12 if tp.get('eligible') else 0) + (8 if x.get('signal_tier') in ('SUPER_LONG','SUPER_SHORT') else 0)
        grade='A' if score>=55 and tp.get('eligible') else 'B' if score>=35 else 'C'
        out.append({'asset':x.get('asset'),'horizon':x.get('horizon'),'meta_decision':d,'meta_score':round(score,1),'grade':grade,
                    'entry_quality':x.get('entry_quality'),'trade_plan_eligible':bool(tp.get('eligible'))})
    out.sort(key=lambda x:x['meta_score'],reverse=True)
    return {'generated_at':now(),'opportunities':out,'top':out[0] if out else None,'mode':'live_lightweight'}
...
def product_overview():
    ...
    if cached:
        ...
        out['learning_progress']=learning_progress(); out['users']=user_metrics(); out['signal_capacity']=signal_capacity_status()
...
def main():
    ...
    emit('product_ready', dashboard='/app', api='/api/v1/overview', history_api='/api/v1/history',
         ...
         expert_replay_api='/api/v1/expert-replay', expert_policy_api='/api/v1/expert-policy', decision_journal_api='/api/v1/decision-journal', structure_analogs_api='/api/v1/structure-analogs',
